...
"""
Note - All mzip saves are converted to normal pickle just for the algorithm run
"""
...
def GKTL_run(k_val):
    ...
    initial_sample = random.sample((range(1,3721)),Num_traj)

    #setting up trajectory and global arrays
    for i in range(Num_traj):
        with mgzip.open('../initial/state'+str(initial_sample[i]), 'rb') as f:
            Data_copy = pickle.load(f)
        with open('traj'+str(i+1), 'wb') as f:
    ...
    with mgzip.open('stats50_k'+str(k_val)+'b', 'wb') as f:
